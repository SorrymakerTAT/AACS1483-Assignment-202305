...
int main()
{
	...
	char enteredid[MAX_ID_LENGTH], staffname[MAX_ID_LENGTH], studentname[MAX_ID_LENGTH], studentID[MAX_ID_LENGTH], semesterSession[MAX_ID_LENGTH];
	...
	while (1)
	{
		...
		else
		{
			...
			if (strcmp(enteredid, STAFFID) == 0)
			{
				...
				scanf("%s", &studentID);
				rewind(stdin);
				printf("3.Course Intake : ");
				scanf("%d", &courseintake);
				printf("\n");
				printf("Student found\n");
				printf("1.Name: %s 2.Student ID : %s 3.Course Intake : %d \n", studentname, studentID, courseintake);
				printf("\n");
				...
				for (int i = 1; i <= courseintake; ++i)
				{
					...
					scanf("%s", &grade);
					rewind(stdin);
					printf("4.Current semester session : ");
